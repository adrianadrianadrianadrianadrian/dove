...
use crate::cbs::{put_sas_token, CbsOptions};
use crate::conn;
use crate::driver::{
...
impl Session {
    ...
    pub async fn with_cbs(&self, cbs_options: CbsOptions) -> &Self {
        match cbs_options {
            CbsOptions::Sas(sas_config) => 
                put_sas_token( &self
                             , &sas_config.address
                             , &sas_config.sasl_key_name
                             , &sas_config.sasl_key
                             , sas_config.token_expiry
                             ).await.unwrap(),
        }
